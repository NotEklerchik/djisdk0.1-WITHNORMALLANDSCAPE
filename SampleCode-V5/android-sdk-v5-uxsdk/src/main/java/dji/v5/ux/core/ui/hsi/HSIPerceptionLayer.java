...
public class HSIPerceptionLayer implements HSIContract.HSILayer {
    ...
    /**
     * Сгруппируйте данные и используйте наименьшее значение в каждой группе.
     * Группировка данных позволяет избежать отображения очень тонких карт препятствий.
     */
    @NonNull
    ...
    /**
     * Определите, попадают ли начальная и конечная позиции данных в слепую зону.
     */
    ...
    /**
     * Попадают ли данные в «слепую зону»?
     */
     ...
     * @param offset Индекс, с которого начинаются данные
     * @param size   Объем данных, который необходимо вычислить
     * @return
     * @throws IndexOutOfBoundsException offset Должно быть меньше date размер
     */
     ...
     * @param data   набор данных
     * @param offset Индекс, с которого начинаются данные
     * @param size   Размер, который необходимо заполнить
     * @return
     * @throws IndexOutOfBoundsException offset должно быть меньше размера date
     */
    ...
    /**
     * Нарисуйте фон дальности обнаружения препятствий в HSI.
     *
     * @param compassSize радиус компаса
     */
    private void drawPerception(Canvas canvas, Paint paint, int compassSize) {
        ...
        // Рисуйте в порядке: спереди, справа, сзади и слева.
    ...
    /**
     * После установки цвета прозрачности кисти необходимо восстановить прозрачность кисти.
     */
    ...
    private void drawBarrier(Canvas canvas, Paint paint, int compassSize) {
        ...
        // Здесь также учитывается отмостка, начиная с середины отмостки, расчет относительно прост.
        int perceptionAngleEach = 360 / DEFAULT_PERCEPTION_AREA_COUNT;

        for (Shape shape : mShapeList) {
            canvas.save();
            // Определить, правильно ли работает сенсорная система в этом направлении.
        ...
        for (int i = 0; i < horizontalBarrierDistance.size(); i++) {
            ...
            if ((distanceInMeter >= visibleDistanceInHsi || i == horizontalBarrierDistance.size() - 1)
                ...
                if (minDistanceInMeter > mHorizontalPerceptionDistance) {
                    ...
                    // feature HYAPP-10551 Объезд препятствий становится красным с [тормозной путь] на [тормозной путь + 2 м]
