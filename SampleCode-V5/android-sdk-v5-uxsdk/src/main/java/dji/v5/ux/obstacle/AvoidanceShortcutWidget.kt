...
/**
 * Description : Среди промышленных машин только M3 поддерживает обход,
 * а другие модели поддерживают только торможение и закрытие.
 * Его можно использовать для быстрой установки всех состояний обхода препятствий,
 * а также остановки/закрытия/замыкания одним щелчком мыши.
 *
 * @author: Byte.Cai
 *  date : 2023/8/11
 *
...
class AvoidanceShortcutWidget @JvmOverloads constructor(
    ...
    override fun onTabSelected(tab: TabLayout.Tab?) {
        ...
        if (value != ObstacleAvoidanceType.CLOSE) {
            ...
            // Если текущая передача — A/S, пользователю необходимо будет указать
            if (mode == "A" || mode == "S") {
    ...
    private fun updateTabItems() {
        ...
        //Добавить параметры настройки поведения при избегании препятствий
        getAPASModeRange().forEach {
            apasTab.addTab(generateTabItem(it), false)
        }

        // Инициализируйте вкладку, а затем установите наблюдателя.
        updateTabByMode()
    }
    ...
    private fun updateSelectTab(type: ObstacleAvoidanceType) {
        for (i in 0 until apasTab.tabCount) {
            ...
            if (tab?.tag == type) {
                selectTab(tab)
                //Обновить текстовое описание соответствующего режима
    ...
    /**
     * Динамическое добавление субэлементов управления APAS
     */
    private fun getAPASModeRange(): List<ObstacleAvoidanceType> {
        //Среди промышленных моделей только М3 поддерживает обход, а остальные модели поддерживают только торможение и закрытие.
